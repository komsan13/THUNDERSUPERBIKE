...
class CalculateController extends Controller
{
    ...
    protected $controller = 'calculate';
    protected $folder = 'calculate';
    protected $menu_id = '5';
    protected $name_page = "Calculate";
    ...
    {
        $like = $request->Like;
        $sTable = CalculateModel::orderby('sort', 'asc')
        ...
        return Datatables::of($sTable)
            ...
            ->addColumn('change_sort', function ($row) {
                $sorts = CalculateModel::orderby('sort')->get();
    ...
    {
        $datas = CalculateModel::find(explode(',', $request->id));
        if (@$datas) {
            foreach ($datas as $data) {
                CalculateModel::where('sort', '>', $data->sort)->decrement('sort');
                $query = CalculateModel::destroy($data->id);
            }
        }
    ...
    {
        $data = CalculateModel::find($request->id);
        $checksort = CalculateModel::where('id','!=',$data->id)->where('sort',$request->sort)->first();
        if($checksort)
        {
            $new_sort = CalculateModel::where('sort',$request->sort)->first();
    ...
    public function store($request, $id = null)
    {
        try {
            ...
            if ($data->save()) {
                DB::commit();
                if($id==null){ CalculateModel::where('id','!=',$data->id)->increment('sort'); }
