...
class TranslateController extends Controller
{
    ...
    protected $folder = 'translate';
    protected $menu_id = '3';
    protected $name_page = "Translate";
    ...
    {
        $like = $request->Like;
        $sTable = TranslateModel::orderby('sort', 'asc')
        ...
        return Datatables::of($sTable)
            ...
            ->addColumn('action_name_en', function ($row) {
                $data = "$row->name_en";
                return $data;
            })
            ->addColumn('updated', function ($row) {
                $data = date('d/m/Y', strtotime('+543 Years',strtotime($row->updated)));
                return $data;
            })
            ->addColumn('change_sort', function ($row) {
                $sorts = TranslateModel::orderby('sort')->get();
            ...
            ->rawColumns(['action_name', 'action_name_en', 'change_sort', 'created', 'action'])
            ->make(true);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view("$this->prefix.pages.$this->folder.edit", [
            ...
            'segment' => $this->segment,
            'name_page' => $this->name_page,
            'row' => TranslateModel::find($id),
        ]);
    }

    public function status($id = null)
    {
        $data = TranslateModel::find($id);
    ...
    {
        $datas = TranslateModel::find(explode(',', $request->id));
        if (@$datas) {
            foreach ($datas as $data) {
                TranslateModel::where('sort', '>', $data->sort)->decrement('sort');
                $query = TranslateModel::destroy($data->id);
            }
        }
    ...
    {
        $data = TranslateModel::find($request->id);
        $checksort = TranslateModel::where('id','!=',$data->id)->where('sort',$request->sort)->first();
        if($checksort)
        {
            $new_sort = TranslateModel::where('sort',$request->sort)->first();
    ...
    public function store($request, $id = null)
    {
        try {
            ...
            if ($data->save()) {
                DB::commit();
                if($id==null){ TranslateModel::where('id','!=',$data->id)->increment('sort'); }
